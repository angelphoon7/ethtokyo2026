...
**Final verdict for the latest resume: PARTIAL — criteria 1–4 PASS, criterion 5 PARTIAL. A separate-process protected signer settled one allowed Base Sepolia payment ([tx 0x306b…ca62](https://sepolia.basescan.org/tx/0x306b625e697942b3eed6a6c5e2f18a999cac7f08e9686fecb7034cc79fc5ca62), 0.01 USDC, HTTP 200 with the paid content) and held every risk, changed-quote, Intercepta-error and bypass case at zero signatures. OS-level key isolation is not provisioned.** See the "Protected-signer resume" section below. The earlier controlled risk-HOLD paragraph below is retained as history.
...
| 2 | Real 402 from a working paid endpoint; allowed payment actually settled | **PASS** | [allowed-payment.json](spike-7c/evidence/x402/allowed-payment.json), [signer audit](spike-7c/evidence/x402/signer-audit.jsonl). 2026-09-26 00:11:09–00:11:12 UTC: live 402 → live Intercepta scan of payee (HTTP 200, score 0, no traits, 768 ms, owner-assumption ALLOW) → one approval → payment header → **HTTP 200** with the "Protected Content — Your payment was successful" page. Facilitator settlement `success: true`, network `eip155:84532`, payer `0x4a59…D9fD`, tx [`0x306b625e…ca62`](https://sepolia.basescan.org/tx/0x306b625e697942b3eed6a6c5e2f18a999cac7f08e9686fecb7034cc79fc5ca62). Independent RPC check: block 47306592, receipt status `0x1`, a matching USDC `Transfer` of 10000 from payer to payee. Payer balance 20,000,000 → 19,990,000 atomic. **Sign count = 1.** | `npm run signer`, then `npm run live:payment` |
...
| 4 | Mutated-quote, unknown/error and bypass cases: zero payer signatures | **PASS** (funded signer for mutation, replay, budget and bypass; ephemeral separate-process signer for Intercepta error; offline controls also pass) | **Funded signer, before the payment:** no-approval and default auto-pay wrapper refused; six mutations of the approved quote each refused (`MUTATED_TRANSFER`×3, `MUTATED_DOMAIN`×2, `MUTATED_VALIDITY`), signature count still 0. **After the payment:** replay `APPROVAL_CONSUMED`, second quote `TASK_BUDGET`, no-approval `NO_GUARD_APPROVAL`, other endpoint `LOCAL_ENDPOINT`, key-export route `NOT_FOUND`. Final signer counters: 11 sign requests, 10 rejected, 1 invoked, **1 signature** (the exact allowed payment). Ephemeral run: six mutations 6 of 6, Intercepta error (invalid key → HTTP 403) → HOLD `RISK_UNKNOWN`. Bypass: no approval, forged approval id, default auto-pay wrapper, agent purchase tool and a key-export route all yield zero signatures. Offline: 10 signer controls, plus the historical suite, pass. Earlier ephemeral detail: six post-approval mutations (`payTo`, `amount`, `asset`, `network`, validity, sender) each refused, 6 of 6, signatures 0. Intercepta error (invalid key → HTTP 403) → HOLD `RISK_UNKNOWN`, signatures 0. Bypass: no approval, forged approval id, default auto-pay wrapper, agent purchase tool and a key-export route all yield zero signatures. Offline: 10 signer controls, plus the historical suite, pass. | `npm run live:negative`, `npm run live:payment`, `npm run test:signer` |
...
- **Risk-hold on the funded key was not run.** The funded signer's policy is least-privilege: one endpoint and one recipient, the public demo payee. The live risk HOLD and Intercepta-error cases therefore ran against the same signer code with a generated, unfunded, in-memory key that the launcher never sees. Both share one code path, but the risk-hold outcome was not observed on the funded key itself, and adding the risky fixture to the funded policy just to test it was not done.
- **Isolation is process-level only** (criterion 5, below).
- The owner's zero-score/empty-traits ALLOW rule is still a demo assumption, not proof that the payee is safe or covered. The payment went to the public demo payee `0x2096…287C`, whose only evidence is that rule.
- The paid output is the sponsor-neutral x402 demo page; its usefulness as a real task result is nominal.
- **Budget:** the signer keeps its budget in memory. Its 0.01 USDC per-call and task budget is now consumed; restarting the signer resets it. Spend for the spike: **0.01 test USDC**, well under the owner's 0.1 USDC cap.

**Handling notes.** `npm run signer` first exited `PAYER_KEY_MISSING_OR_MALFORMED` because the key was 64 hex characters without a `0x` prefix. The signer now accepts both forms. To diagnose this, only the shape of the key value (length, prefix, character set) was inspected, and the value itself was never printed, logged or committed. The agent-side runner asserted no payer key in its own environment.
...
**Payer key.** The owner placed the key for `0x4a59…D9fD` in the Git-ignored `spike-7c/signer/.env.signer`. The signer verified that it derives to the expected address before listening and ran with `ephemeral: false`. The signer process was stopped after the run.

**Smallest remaining step.** Only criterion 5's hardening: run the signer under a separate OS account or container so that shell access alone cannot read its key file, then rerun `npm run live:payment` against it.

### Remaining dependencies and final disposition

1. ~~Payer key placement~~ — done. Public address, funding and cap were supplied; the key is in the signer's env file only.
2. ~~One real allowed payment~~ — done: settled, verified on-chain, usable paid output. See criterion 2.
3. **Optional hardening for criterion 5** (above). Whether process-level isolation suffices for the sponsor demo is the owner's call.
...
**PARTIAL — criteria 1–4 PASS, criterion 5 PARTIAL.** [07D](07D_GO_NO_GO_DECISION.md)'s condition 2 (allowed purchase settles through a protected funded signer and delivers usable output) is met at a process boundary; condition 3 (held, changed-quote and bypass attempts at zero signatures, no alternate funded route for the agent) is met for changed-quote and bypass on the funded signer and for risk-hold on the same code with an ephemeral key. Whether that satisfies the owner's conditional GO, including the same-OS-user isolation limit, is the owner's decision and is not made here. Customer need remains unproven, and no full product build was started.

**One next action:** decide whether process-level isolation is enough for the limited sponsor demo, or provision a separate OS account or container for the signer and rerun the payment.
